...
// Endpoint: https://[PROJECT_REF].supabase.co/functions/v1/whatsapp-webhook
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (isValidMode && isValidToken) {
        ...
        // O Content-Type deve ser text/plain e a resposta deve ser EXATAMENTE o challenge
    ...
    if (req.method === "POST") {
      ...
      // ============================================
      // VERIFICAR SE É EVENTO DE STATUS DE TEMPLATE
      // ============================================
      ...
      if (field === "message_template_status_update") {
        ...
        // Extrair dados do payload conforme documentação da Meta
        // Estrutura: value.message_template_id, value.event, value.reason
        const templateId = value?.message_template_id
        const event = value?.event // 'APPROVED', 'REJECTED', 'PENDING', 'PAUSED', 'PENDING_DELETION'
        const reason = value?.reason // Motivo da rejeição (apenas se REJECTED)
        ...
        const updateData: any = {
          status: event.toUpperCase(), // Garantir que está em maiúsculas
          atualizado_em: new Date().toISOString()
        }
        
        // Adicionar motivo_rejeicao apenas se o evento for REJECTED e houver reason
        if (event.toUpperCase() === "REJECTED" && reason) {
          updateData.motivo_rejeicao = reason
          console.log("[Webhook POST] Motivo da rejeição:", reason)
        } else {
          // Se foi aprovado ou mudou para outro status, limpar motivo_rejeicao
        ...
        // Atualizar status na tabela modelos
        ...
        // Retornar OK para a Meta parar de reenviar o evento
        return new Response("OK", { 
          status: 200, 
          headers: corsHeaders 
        })
      ...
      // Buscar conexão - tentar como string primeiro
      ...
      console.log("[Webhook POST] Primeira busca (string):", {
        ...
        // Se não encontrou, tentar sem filtro de status
      ...
      // Se ainda não encontrou, listar TODAS as conexões para debug
      if (conexaoError || !conexao) {
      ...
      // Obter email_usuario da conexão (opcional - não é mais obrigatório)
      ...
      for (const message of messages) {
        ...
        try {
          ...
          // Não bloquear o processamento se falhar ao buscar foto
        }
        ...
        if (contatoExistente) {
          ...
          // Atualizar último acesso e foto de perfil (usar apenas atualizado_em, não ultima_mensagem_em)
          console.log("[Webhook POST] ====== ATUALIZANDO CONTATO EXISTENTE ======")
          const updateData: any = {
            atualizado_em: new Date().toISOString(),
            nome: contactName || contatoExistente.nome // Atualizar nome se vier do WhatsApp
          }
          
          // Atualizar foto de perfil se encontrada
          if (fotoPerfil) {
          ...
          // Preparar dados do contato (email_usuario é opcional)
          console.log("[Webhook POST] ====== CRIANDO NOVO CONTATO ======")
          const dadosContato: any = {
            id_empresa: idEmpresa, // OBRIGATÓRIO - associar à empresa
            nome: contactName || phoneNumber,
            telefone: phoneNumber,
